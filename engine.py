...
def run_engine(df: pd.DataFrame, p) -> pd.DataFrame:
    ...
    df["equity_usd"] = df["exposure_usd"] * df["equity_pct"] * df["s3123_factor"]
    ...
    df["s3123_eligible"] = df.apply(
        lambda r: bool(r["is_consortium"]) and r["spacecraft_id"] not in q["excluded"],
        axis=1)
    def _s3123(row):
        if not row["s3123_eligible"]:
            return 0.0
        if not (q["date_from"] <= row["inception"] <= q["date_to"]):
            return 0.0
