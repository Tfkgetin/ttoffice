...
def _summary(ws, grid, params, source, per_layer):
    ...
    # column letters: B Scenario, C Detail, D Gross, E ExtQS, F NetExtQS,
    #                 G IGRQS, H NetQS, I XoL, J Net, K Net/Gross
    r = 5
    for entity in ["FIHL", "FUL", "FIID", "FIBL"]:
        ws.cell(row=r, column=2, value=entity).font = F_SECT
        _desc = {"FIHL": "group / consolidated — incl. S3123 QS + IG Equity",
                 "FIBL": "internal RI receiver — cessions in (gross ≈ net)",
        ...
        r = _hdr(ws, r + 1, 2, headers, widths)
        ...
        for k, (_, row) in enumerate(block.iterrows()):
            ...
            _cell(ws, r, 4, val("gross"), alt=alt, money=True)
...
def _changes_narrative(sc):
    ...
    bits = []
    # FIBL is the IGR receiver — increases are structural cessions, not new risk
    if (sc["entity"] == "FIBL").any():
        fibl = sc[sc["entity"] == "FIBL"]
        if (fibl["d_gross_pct"].fillna(0) > 0).any():
            bits.append("FIBL is the IGR receiver — its increases reflect ceded "
                        "volume (structural), not new gross risk.")
    # FIID running off?
...
def _changes(wb, changes):
    ...
    if ents:
        _changes_matrix_live(ws, HEADLINE_ROW, sc, ents, scens_present,
                             gross_row, gross_first, gross_last,
                             "Movement by scenario & entity  (Gross Δ% vs prior quarter)")
...
def _cover(wb, params, per_layer, grid):
    ...
    fihl_sw = grid[(grid["entity"] == "FIHL") &
                   (grid["scenario"] == "Space Weather")]["gross"]
    fihl_worst = float(fihl_sw.iloc[0]) if len(fihl_sw) else 0
...
def _exec_summary(wb, grid, per_layer, params, source, recon, excluded, changes):
    ...
    fihl_gross = float(fihl_sw["gross"].iloc[0]) if len(fihl_sw) else 0
    fihl_net = float(fihl_sw["net"].iloc[0]) if len(fihl_sw) else 0
    ...
    for ent in ["FIHL", "FUL", "FIID", "FIBL"]:
        ...
        money_m_cell(r, 4, float(worst["gross"]), alt=alt)
        net_val = worst["net"] if pd.notna(worst["net"]) else None
        if net_val is not None:
            money_m_cell(r, 5, net_val, alt=alt, bold=True)
            _cell(ws, r, 6, net_val / worst["gross"] if worst["gross"] else 0,
                  alt=alt, pct=True)
        else:
    ...
    ws.cell(row=r, column=2,
            value=f"▸  {binding} is the worst-case scenario across all entities. "
                  ...
                  f"retention; FIHL group gross {_money_m(headline['FIHL']['gross'])}.")
...
def write_results(path, per_layer, sw, mr, grid, params, source,
    ...
    if "Sheet" in wb.sheetnames:
        del wb["Sheet"]
    order = ["Cover", "Executive Summary", "Summary", "Charts",
