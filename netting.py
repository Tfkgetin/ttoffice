...
def _fihl_fibl(df: pd.DataFrame, m: pd.Series, p) -> dict:
    ...
    Verified to the dollar against the manual Netting Waterfall (combined table):
      ...
      FIBL = QS ceded (from FUL+FIID) + XoL ceded + direct FIBL + S3123 + Equity
             (the internal receiver; gross ≈ net, only XoL recoveries differ)
    ...
    # --- FIHL: group gross/net ---
    all_gross = (w * df["per_sc"]).sum()
    all_ext_qs = (w * df["ext_qs"]).sum()
    all_net_ext = (w * df["net_ext"]).sum()
    fihl_gross = all_gross + s3123 + equity
    # group net of external QS (manual G5 path uses net-of-ext-QS BF) + IG add-ons
    fihl_net = all_net_ext + s3123 + equity
    fihl = {
        "entity": "FIHL", "gross": fihl_gross, "ext_qs": all_ext_qs,
        "net_of_ext_qs": all_gross - all_ext_qs,
        "igr_qs_ceded": 0.0, "net_of_qs": fihl_net,
        "xol_ceded": 0.0, "net": fihl_net,
        "s3123_qs": s3123, "equity_usd": equity,
    }

    # --- FIBL: internal reinsurance receiver ---
    ...
    fibl_gross = qs_ceded_in + xol_ceded_in + direct_fibl + s3123 + equity
    # FIBL retains the cessions it receives; its only net leakage is the external
    # QS on its own DIRECT book (gross per_sc → net_ext). Mirror the manual:
    # net swaps direct gross for direct net-of-external-QS.
    fibl_net = qs_ceded_in + xol_ceded_in + direct_fibl_net + s3123 + equity
    fibl = {
        "entity": "FIBL", "gross": fibl_gross, "ext_qs": 0.0,
        "net_of_ext_qs": fibl_gross,
        "igr_qs_ceded": 0.0, "net_of_qs": fibl_net,
        "xol_ceded": direct_fibl - direct_fibl_net, "net": fibl_net,
        "s3123_qs": s3123, "equity_usd": equity,
