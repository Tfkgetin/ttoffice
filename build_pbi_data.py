...
def main():
    ...
    pd.DataFrame([
        ...
        dict(scenario="Generic Defect", loss_factor="0.50", orbit_scope="GEO-GSO, MEO",
             ...
             basis_short="50% × RPF, GEO-GSO + MEO", scenario_type="Damage ratio"),
        dict(scenario="Space Debris",   loss_factor="LEO .40 / MEO .10 / GEO .05",
             orbit_scope="All", uses_rpf=False, selection="Damage ratio by orbit",
             method_desc="Orbit-tiered factors: 40% LEO, 10% MEO, 5% GEO. "
                         "Driver is the orbit mix, dominated by the LEO book.",
             basis_short="40% LEO / 10% MEO / 5% GEO, all orbits", scenario_type="Damage ratio"),
        dict(scenario="Space Weather",  loss_factor="1.00", orbit_scope="All",
             uses_rpf=False, selection="Total loss, manufacturer fleet",
             method_desc="Total loss of every spacecraft built by the single worst "
                         "bus-manufacturer group. A total-loss selection rather than a "
                         "damage ratio — driver is manufacturer concentration.",
             basis_short="100% of worst bus-manufacturer group", scenario_type="Total loss"),
        dict(scenario="Max Risk",       loss_factor="1.00", orbit_scope="All",
             uses_rpf=False, selection="Total loss, largest single risk",
             method_desc="Total loss of the single largest spacecraft exposure. "
                         "A total-loss selection rather than a damage ratio — driver is "
                         "the peak single risk.",
             basis_short="100% of largest single spacecraft", scenario_type="Total loss"),
